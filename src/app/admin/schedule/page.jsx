...
const fetchSchedulePageData = async () => {
    ...
    try {
                ...
                if (timeSlotName) {
                    schedule.dayDetails.forEach(dayDetail => {
                        ...
                        if (dayAbbr) {
                            ...
                            // Ensure we're including the scheduleId in the mapping
                            scheduleMap[dayAbbr][timeSlotName][schedule.roomId] = {
                                classId: schedule.classId,
                                scheduleId: schedule.scheduleId, // Make sure this is included
                                className: schedule.className,
                                majorName: schedule.majorName,
                                year: schedule.generation // Include generation/year if needed
                            };
                            assignedClassIds.add(schedule.classId);
                        }
                    });
                }
            }
        ...
        const finalClasses = [
            ...transformedClasses.filter(cls => !assignedClassIds.has(cls.classId)),
            ...unassignedClasses
        ];
